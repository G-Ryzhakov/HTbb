...
def build_cross(func, m, d, n, e=1.0e-16, nswp=100, seed=0):
...
def build_our(func, nswp, d, n, r, dr_max, with_balanced=True, seed=0):
    @my_cach(cache_size=1.0e7, cache_max=1.0e9, check_cach=False)
    def func_cache(I):
...
def run_one(
    ...
    for Bm in BMS:
        bm = Bm(n=n, d_x=d, d_y=d, r_x=n_inner, r_y=n_inner, seed=bm_seed)
        bm.prep()
        ...
        inp_tens_mix = rand.permutation(np.arange((d**2)))
        ...
        I_tst = teneva.sample_rand([n] * (d**2), m_tst, seed=0)
        y_tst = func(I_tst)

        _t = tpc()
        info["m"] = 0
        getter = build_our(func, nswp, (d**2), n, r, dr_max, with_balanced, seed)
        t = tpc() - _t
        ...
        m = info["m"]
        info["m"] = 0
        getter = build_cross(func, m, (d**2), n, seed=seed)
        t = tpc() - _t
...
def _args_build():
    ...
    parser.add_argument("--d", type=int, help="Dimension of the problem", default=256)
    parser.add_argument("--n", type=int, help="Mode size for each dimension", default=8)
    parser.add_argument("--n_inner", type=int, help="no", default=8)
    parser.add_argument(
        "--nswp", type=int, help="Number of sweeps for the HT-method", default=100
    )
    parser.add_argument("--r", type=int, help="Rank for the HT-method", default=2)
    parser.add_argument(
        "--dr_max", type=int, help="Rank increment for the HT-method", default=1
    )
    parser.add_argument(
        "--with_balanced",
        action="store_false",
    ...
    parser.add_argument(
        "--m_tst", type=float, help="Number of test points", default=1.0e4
    )
    parser.add_argument(
        "--fold",
        type=str,
    ...
    parser.add_argument(
        "--postfix", type=str, help="Optional postfix for file names", default=None
    )
    parser.add_argument(
        "--gpu", type=str, help="gpu for script to run in", default=None
    )
    parser.add_argument("--bmseed", type=int, help="bm seed", default=42)
    parser.add_argument(
        "--seed_half",
        action="store_true",
...
if __name__ == "__main__":
    ...
    if is_show:
        show(without_bs=args[7], seed_half=args[-1])
    else:
        run(*args)
